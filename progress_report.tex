...
T6 Push-T, precision regime & $+2.3$ to $+4.5 \pm 6.2$; faster (80.5 vs 93 steps) & \open{} --- 4$\times$-power rerun \\
\bottomrule
\end{tabular}
\end{center}
...
ceiling. (2)~\textbf{Free correction erases knowledge}: T6's blind pusher
identifies the physics \emph{by watching the object respond}, so explicit
knowledge only pays where tolerances are tight enough that a wrong-model push
near the goal costs a full correction cycle. T6 (\texttt{DynPushT6-v1}) is
modeled on the established Push-T benchmark with hidden mass/friction/COM on the
tee --- the first task where the COM axis is decisive, and the only one whose
controller family worked on the first probe (50\% nominal).
...
\item \open{} T6 high-power precision gate ($\pm$3.1 CI); if the premium holds,
T6 gets the same automated pipeline (teacher $\to$ datasets $\to$ arms).
