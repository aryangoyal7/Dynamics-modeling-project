...
\section{First main-task evidence: A vs B at $10^3$ demonstrations}
All \textbf{120 T3 arms} at the $10^3$/$10^4$ scales finished their 150k
training steps with zero failures (checkpoints on persistent storage). The
first preregistered main-task comparison is in --- \textbf{10 seeds of A vs 10
seeds of B-multistep at the $10^3$ scale}:

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Arm} & \textbf{All-tag degradation slope} & \\
\midrule
A (no prediction head) & $+0.0157 \pm 0.0017$ & \\
B-multistep & $+0.0131 \pm 0.0019$ & directionally $\sim$17\% shallower \\
B-onestep (2 seeds so far) & $+0.0145 \pm 0.0039$ & \\
\bottomrule
\end{tabular}
\end{center}

The 95\% CIs overlap on every tag, so under the preregistered rule the
verdict at this scale is \textbf{effect absent} --- reported as such. The
overlap on the headline ``all'' tag is marginal (the intervals nearly touch,
Figure~\ref{fig:t3result}), which is exactly the situation the $10^4$ and
$10^5$ tiers exist to resolve: if the prediction head helps, its advantage
should grow (or vanish) with data, and both arms' remaining variants
(B-latent, B-shuffled, C) complete the causal picture. Those evaluations are
running now.

\begin{figure}[h]
\centering
\includegraphics[width=0.98\textwidth]{figs/t3_1e3_result.png}
\caption{First main-task result, $10^3$ scale. Left: success under
increasing physics shift $\delta$ (mean over seeds, binned over the
$\delta$-grid). Right: per-seed degradation slopes, mean $\pm$ 95\% t-CI
(interpolation tag omitted: too few grid points for a stable per-seed fit).
A vs B-multistep CIs overlap $\Rightarrow$ no effect at this scale under the
preregistered rule.}
\label{fig:t3result}
\end{figure}
