...
A task earns its place here only if it has this property: \textbf{the outcome depends on physics that unfolds after the policy has committed, and nearby parameter values demand qualitatively different behaviour rather than scaled versions of the same behaviour.} A task that fails this test can be solved by interpolating over the training range without modelling anything, which is why simple planar pushing was discarded: friction on a flat table is a first-order effect and a policy covers it by memorising a smooth mapping.

Three tasks, chosen so that each tests a different mechanism rather than three variants of one.

\begin{tabular}{@{}p{2.5cm} p{4.6cm} p{4.0cm} p{5.0cm}@{}}
\toprule
Task & What the robot does & Hidden parameters & Why the dynamics bind \\
\midrule
T1, pouring & Tilt a container to transfer contents into a target vessel & fill level, content friction, container inertia & Outflow lags the tilt, so the policy commits to a tilt rate before observing its effect. Rate is nonlinear in fill level \\
T2, carrying & Move a mug held by its handle without spilling or dropping & fill level, content friction, handle offset & The robot's own acceleration excites the disturbance it must then damp. Its past actions create the problem it now faces \\
T3, slide to slot & Slide an object into a slot, recovering when it stops short or overshoots & surface friction, object mass, COM offset & Friction sets stopping distance. Stop short and you re-approach and push again; overshoot and you must come around and push back. The recovery \emph{direction} flips with friction \\
\midrule
T4, control & Pick and place a rigid object & same as T3 & Once grasped, the object is rigid to the gripper and its properties stop mattering. Effect should be near zero \\
\bottomrule
\end{tabular}

T3 is the primary task and should be built first: it is rigid-body throughout, needs no special solver, and its recovery-direction property is the cleanest scientific structure in the set. T1 and T2 are the harder tier and follow once T3 reports.

\textbf{Why the mechanisms differ.} T1 tests anticipation under delayed consequence. T2 tests closed-loop damping of a self-induced disturbance. T3 tests inference followed by a discrete decision whose correct answer inverts across the parameter range. A dynamics-prediction objective could plausibly help with all three for different reasons, and if it helps with only some, that pattern is itself a finding.

\textbf{Build note on contents.} Use granular contents, roughly twenty to thirty small rigid spheres, rather than true fluid. Fluid requires a material point or particle solver whose availability in ManiSkill 3 should be verified before committing, whereas granular contents are plain rigid-body physics and retain the shifting-mass and delayed-outflow behaviour that makes T1 and T2 worth running. The hidden parameters then become particle count, inter-particle friction, and the container's centre of mass.

\textbf{Extra metrics these tasks afford.} T3 in particular supports a measurement the pushing task did not: whether the \emph{first} recovery attempt moved in the correct direction. That isolates the inference from the execution, since a policy can identify the friction correctly and still execute the push poorly. Log it separately. T1 affords spill volume and transfer accuracy as continuous measures; T2 affords peak content displacement, which measures the disturbance directly rather than through task success.
...
\begin{itemize}
\ck Install ManiSkill 3. Confirm a stock task trains end to end with the PPO baseline before changing anything.
\ck Write the T3 environment first: object, table, slot, and a success condition that permits multiple attempts. Log attempt count and the direction of the first recovery push.
...
\ck Write the $c$ sampler: log-uniform over mass multiplier $[0.7, 1.4]$, surface friction $[0.3, 0.7]$, COM offset up to 15 percent of half-width. For T1 and T2, add particle count and inter-particle friction.
...
\ck Add T1 and T2 once T3 works, using granular contents rather than fluid. Add T4, pick and place, as the specificity control.
...
\ck Generate the vision variant on T3 at $10^4$ only. Storage binds here, not compute.
...
\ck Check the effect across T1, T2, T3 and the T4 control. It should be substantial on T1 to T3 and near zero on T4. If the effect differs between T1, T2 and T3, that pattern says which mechanism the prediction objective actually serves, which is more informative than a single averaged number.
